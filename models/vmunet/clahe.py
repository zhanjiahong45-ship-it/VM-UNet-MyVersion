...
class FrontendEnhancement(nn.Module):
    r"""
    硬件级网络最前端滤镜：针对浅色病灶增强 (Gamma 3.5 + CLAHE 7.0)
    纯 PyTorch + Kornia 实现，全流程在 GPU 上运行，绝不阻塞计算图。
    """
    ...
    def forward(self, x):
        # 假设输入张量 x 的范围在 [0, 1]，形状为 [B, 3, H, W]
        # 1. 转换到 LAB 空间
        x_lab = kornia.color.rgb_to_lab(x)

        # 2. 分离通道 (Kornia中 L 范围约[0, 100])
        L = x_lab[:, 0:1, :, :]
        A = x_lab[:, 1:2, :, :]
        B = x_lab[:, 2:3, :, :]

        # 3. L 通道归一化到 [0, 1] 以供后续非线性计算
        L_norm = L / 100.0
        L_norm = torch.clamp(L_norm, 1e-6, 1.0)

        # 4. 在 L 通道执行 CLAHE 对比度受限自适应直方图均衡
        try:
            # kornia.enhance.equalize_clahe 要求张量在 [0, 1] 之间
            ...
            # 兼容性后备方案：如果环境中 Kornia 版本较老没有 equalize_clahe
            L_clahe = kornia.enhance.equalize(L_norm)

        # 5. 执行 Gamma 3.5 压暗操作 (强化浅色病灶边缘)
        L_gamma = torch.pow(torch.clamp(L_clahe, min=1e-6), self.gamma)

        # 6. 还原 L 通道数值尺度
        L_final = L_gamma * 100.0

        # 7. 拼接回 LAB 空间并转换回 RGB
        lab_enhanced = torch.cat([L_final, A, B], dim=1)
        x_enhanced = kornia.color.lab_to_rgb(lab_enhanced)

        # 浮点安全锁，防止极值引发 NaN
        return torch.clamp(x_enhanced, 0.0, 1.0)
