...
# =========================================================================
# [IMPROVED] GDM Module: Gradient-guided Dynamics Modulation (Lite MLP Version)
# =========================================================================
class GDM(nn.Module):
    def __init__(self, dim, dt_rank, ratio=4):
        """
        GDM (Lite): Gradient-guided Dynamics Modulation with Bottleneck MLP.

        Solves the monotonicity problem of the linear version by introducing
        non-linearity, while keeping parameter count low using a bottleneck structure.

        Args:
            dim: Input channel dimension (C)
            dt_rank: Rank of Delta (C/16)
            ratio: Bottleneck reduction ratio (default 4) to save FLOPs
        """
        super().__init__()

        hidden_dim = int(dim // ratio)

        # 1. Spatial Sensor: 3x3 DW-Conv to capture local gradients
        self.conv = nn.Conv2d(
            in_channels=dim,
            out_channels=dim,
            kernel_size=3,
        ...
        # 2. Pattern Analyzer (Bottleneck MLP): The "Brain"
        # Can learn non-monotonic mappings (e.g., enhance weak edges, suppress strong noise)
        self.mlp = nn.Sequential(
            nn.Linear(dim, hidden_dim),  # Down-project
            nn.LayerNorm(hidden_dim),  # Stabilize
            nn.GELU(),  # Non-linearity
            nn.Linear(hidden_dim, dt_rank)  # Map to Delta rank
        )

        # 3. Zero Initialization
        # Crucial to ensure the model starts exactly like the pre-trained baseline
    ...
    def forward(self, x):
        ...
        Returns: (B, H, W, rank)
        """
        # Step 1: Extract Spatial Gradient
        g = self.conv(x)  # (B, C, H, W)
        g = self.act_conv(g)

        # Step 2: Permute for MLP processing
        g = g.permute(0, 2, 3, 1)  # (B, H, W, C)

        # Step 3: Non-linear Mapping
        dt_gdm = self.mlp(g)  # (B, H, W, rank)
...
class SS2D(nn.Module):
    def __init__(
        ...
        # [ADDITION] Initialize Optimized GDM Module
        self.gdm = GDM(dim=self.d_inner, dt_rank=self.dt_rank, ratio=4)
    ...
    def forward_corev0(self, x: torch.Tensor):
        ...
        # Transform (B, H, W, rank) -> (B, rank, H, W) for manipulation
        ...
        # [ADDITION] Fusion
        # ========================================================
        # Fuse GDM delta with main path delta
        dts = dts + dts_gdm
        # ========================================================
