...
class FCD_Module(nn.Module):
    r"""
    FCD-Module: Score-Maxing Version (暴力刷分版)
    融合了 Dead-Zone 保护与非线性 LAB 空间强光放大的满血版本。
    """
    ...
    def __init__(self, patch_size=4, in_chans=3, embed_dim=96, norm_layer=None, **kwargs):
        ...
        # 死区保护：0.02 以下的差异认为是浅色病灶边缘，绝对不碰！
        self.dead_zone = 0.02

        # 2. 局部对比度放大器 (高起步，直接给 3.0 的暴击)
        self.alpha = nn.Parameter(torch.tensor([3.0]))
        self.local_kernel = 31
        self.local_pad = self.local_kernel // 2

        # 3. 色彩非线性映射 (这是 90.04% 的核心功臣！)
    ...
    def clean_phase(self, x):
        ...
        # 边缘保护：差值减去 0.02，保护微弱病灶边缘，只抓真正的高频毛发
        hair_diff = torch.abs(x_gray - erosion_gray)
        hair_mask = torch.clamp((hair_diff - self.dead_zone) * 20.0, 0.0, 1.0)
        ...
        # 【神级防崩溃锁】：最低值卡在 1e-4，彻底根绝后续 LAB 转换的 NaN 炸弹
        return x_cleaned.clamp(1e-4, 1.0)

    def enhance_phase(self, x_cleaned):
        """阶段 B: 局部细节暴力提取 + LAB 非线性注意力"""
        # 1. 暴力提取并放大局部细节 (Unsharp Masking)
        local_mean = F.avg_pool2d(x_cleaned, kernel_size=self.local_kernel, stride=1, padding=self.local_pad)
        local_detail = x_cleaned - local_mean
        x_stretched = x_cleaned + self.alpha * local_detail

        # 防崩溃锁
        x_stretched = x_stretched.clamp(1e-4, 1.0)

        # 2. 转换到最符合人眼的 LAB 空间 (此时已有 1e-4 垫底，绝对安全)
        x_lab = kornia.color.rgb_to_lab(x_stretched)
        ...
        feat_enhanced = torch.pow(feat_color + 1e-4, gamma_safe)
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
        ...
        # 【核心修改点】直接将 FCD_Module 命名为 patch_embed，使其能够完全替代原版的 PatchEmbed2D 并完美加载预训练权重
    ...
    def forward_features(self, x):
        skip_list = []
        # 直接使用包含 FCD 的 patch_embed 进行前向传播
