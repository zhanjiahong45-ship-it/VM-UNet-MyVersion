...
if __name__ == '__main__':
    config = setting_config() if callable(setting_config) else setting_config

    # =========================================================================
    # 核心劫持：强制将数据集替换为 ISIC17，无需修改 config_setting.py
    # =========================================================================
    config.datasets = 'isic17'
    config.data_path = './data/isic17/'

    # 重新生成实验保存文件夹名称，确保后缀是 isic17 而不是 isic18
    config.work_dir = 'results/' + config.network + '_' + config.datasets + '_' + datetime.now().strftime(
        '%A_%d_%B_%Y_%Hh_%Mm_%Ss') + '/'

    # 【非常关键】：重新初始化 Transformer。
    # 因为 config_setting 里 import 时已经用 'isic18' 初始化了 myNormalize，
    # 如果不在这里重写，预处理阶段会使用 18 的均值方差来处理 17 的图片！
