...
def main(config):
    ...
    # 注意：这里 num_workers 建议设为 4 或 8，设为 0 会比较慢
    ...
    for epoch in range(start_epoch, config.epochs + 1):
        ...
        # [核心修复 1] 使用元组解包 (images, labels) 适配 NPY_datasets
        for i, (images, labels) in enumerate(pbar):
            # [核心修复 2] 强制转 float，解决 Input type (double) 报错
            ...
            # Forward (V3 返回元组)
            outputs, edge_preds = model(images)
        ...
        with torch.no_grad():
            ...
            for images, labels in val_loader:
                ...
                outputs, _ = model(images)  # 验证时忽略边缘输出
        ...
        if avg_val_loss < min_loss:
            ...
            min_epoch = epoch
            print(f">>> Best Model Saved at Epoch {epoch}: Loss {min_loss:.4f}")
